...
# 选择数据库（开发环境选SQLite，生产环境选PostgreSQL）
```

创建 `backend/prisma/schema.prisma`（参考数据库设计文档）
...
# 百度AI配置（植物识别功能）
BAIDU_AI_API_KEY=your_baidu_api_key_here
BAIDU_AI_SECRET_KEY=your_baidu_secret_key_here
BAIDU_AI_PLANT_URL=https://aip.baidubce.com/rest/2.0/image-classify/v1/plant

# 识别功能配置
IDENTIFICATION_CACHE_TTL=86400
MAX_IDENTIFICATION_IMAGE_SIZE=4194304
IDENTIFICATION_TEMP_DIR=uploads/identifications
BAIDU_AI_QPS=2
BAIDU_AI_TIMEOUT=10
...
- **QPS限制**：免费版2 QPS（每秒2次请求）
- **付费升级**：如需更高QPS或更多调用次数，可升级到付费版
...
创建测试脚本 `backend/test_baidu_ai.py`：
...
# 初始化客户端
client = AipImageClassify(API_KEY, SECRET_KEY, SECRET_KEY)
...
def test_identify():
    ...
    # 调用API
    result = client.plant(image_data, baike_num=1)
    print(result)
...
检查 `vite.config.ts` 中的proxy配置，确保后端已启动。
