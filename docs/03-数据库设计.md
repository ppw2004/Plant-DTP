...
│ id           │         │ id           │◄────────│ id           │
│ username     │         │ name         │         │ name         │
│ email        │         │ description  │         │ species      │
│ password     │         │ location     │         │ description  │
│ created_at   │         │ user_id      │         │ image_url    │
│ updated_at   │         │ created_at   │         │ room_id      │
...
3. **Room 1:N Plant** - 一个房间可以有多株植物
4. **Plant 1:N PlantImage** - 一株植物可以有多张图片
5. **Plant 1:N PlantConfig** - 一株植物可以有多个养护配置
6. **Plant 1:N CareLog** - 一株植物可以有多条养护记录
7. **TaskType 1:N PlantConfig** - 一个任务类型可用于多个植物
8. **TaskType 1:N CareLog** - 一个任务类型可以有多条记录
9. **CareLog 1:N CareLogImage** - 一条记录可以有多张图片

### 4.2 级联规则
- **CASCADE**：删除用户/房间/植物时，自动删除关联数据
- **SET NULL**：删除房间时，植物的room_id设为NULL
